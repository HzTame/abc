...
const demoItems = [
  {
    ...
    size: "18 MB",
    downloads: 428,
    coverUrl: "./assets/drumshare-hero.png",
    pattern: "loop",
  },
  {
    id: "plugin-01",
...
async function handleAuthSubmit(event) {
  ...
  if (authMode === "reset") {
    ...
    const { error } = await db.auth.resetPasswordForEmail(email, { redirectTo: passwordRecoveryRedirectUrl() });
    if (error) {
  ...
  const request = isSignup
    ? db.auth.signUp({ email, password, options: { data: { display_name: displayName } } })
...
function authErrorMessage(error, email) {
  const message = error?.message || "ไม่สามารถเข้าสู่ระบบได้";
