(function () {
  ...
  function shouldSkip(payload) {
    ...
    if (recentLogs.size > 80) recentLogs.delete(recentLogs.keys().next().value);
    return now - previous < 4000;
  }
  ...
  async function record(item, fileName) {
    ...
    if ((!payload.asset_id && !payload.asset_title) || shouldSkip(payload)) return;
